...
describe('signin', () => {
  it('redirects directly when using a dev test account', () => {
    ...
    cy.get('input[name=email]').type('testuser+admin@qpayee.com');
  ...
  it('redirects if token is expired but user is already logged in', () => {
    const user = { email: 'testuser+admin@qpayee.com' };
  ...
  it('trims the email when it has trailing spaces', () => {
    cy.visit('/signin');
    cy.get('input[name=email]').type('  user@qpayee.com  ');
    cy.get('input[name=email]').should('have.value', 'user@qpayee.com');
  });
